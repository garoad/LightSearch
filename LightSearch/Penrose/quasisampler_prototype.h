...
public:
  ...
  Point2D(){count = 0; sumX = 0; sumY = 0;}
  Point2D(const double x, const double y) { this->x=x; this->y=y; count = 0; sumX = 0; sumY = 0;}
  Point2D(const double vect[2]) { x=vect[0]; y=vect[1]; count = 0; sumX = 0; sumY = 0;}
...
public:
  ...
  static unsigned getReqSubdivisionLevel( double importance )
  {
  ...
  static unsigned calcImportanceIndex( double importance )
  {
  ...
  static Point2D calcDisplacementVector(double importance, unsigned f_code, int dir)
  {
  ...
  public:
    ...
    Point2D getDisplacedSamplingPoint(double importance)
    {
  ...
  public:
    ...
    void begin(TileNode* s)
    {
      ...
      while ( ! tmp->isTerminal() ) 
		  tmp = tmp->getChild(0); // find first leaf
      shape = tmp;
    }
  ...
  double getImportanceAt_bounded(Point2D pt)
  {
  ...
  void buildAdaptiveSubdivision( unsigned minSubdivisionLevel = 10)
  {
    ...
    do {
      ...
      {   
        ...
        if ( importance >= calcFCodeValue( it->getFCode() , 2*it->getLevel() ) )
        {
          ...
          {
			pt_displaced.level = it->getLevel();
...
public:
  ...
  virtual double getImportanceAt( Point2D pt ) = 0;
  ...
  std::vector<Point2D> getSamplingPoints()
  {
    ...
    buildAdaptiveSubdivision(5); //요게 클수록 느려, 하지만 점의 수가 많지
